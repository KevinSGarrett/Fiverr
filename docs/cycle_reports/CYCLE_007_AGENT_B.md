...
- Added regression coverage for non-positive candidate caps, nested `data-testid` text preservation, and empty fixture payload warning behavior.
...
- `python -m pytest tests/unit/test_collection.py tests/integration/test_collection_e2e.py -q` (pass, 62 passed)
...
- Added unit and integration assertions that increase direct coverage of:
  ...
  - nested `data-testid` extraction preservation for nested markup
