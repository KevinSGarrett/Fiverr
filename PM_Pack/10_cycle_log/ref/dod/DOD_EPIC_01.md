...
| AC ID | Criteria | Validation Method |
|---|---|---|
| AC-1.3.1 | `Base.metadata.create_all(engine)` creates **at least 28 tables** (current implementation creates 30 — 28 spec tables + 2 report tables; see DL-025) (verify with `inspector.get_table_names()`) | Integration test against SQLite |
